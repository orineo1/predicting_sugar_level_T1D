...
async def complex_snooze_finish(context: ContextTypes.DEFAULT_TYPE) -> None:
    """"
    A helper function for complex_snooze, and is part of the flow complex_snooze -> complex_snooze_is_in_condition->complex_snooze_finish.
    ...
    # If the current sugar value is higher than the value defined in complex_snooze, return a message
    if predKalman.currnet_bg_val(collection_mongo,1)>high_val:
        text = "BG level is going over then {}, prediction algorithm set again".format(high_val)
    ...
    text = "BG level is going under then 140, the  algorithm set again"
    await  context.bot.send_message(chat_id, text=text)

async def complex_snooze_is_in_condition(context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    A helper function for complex_snooze, and is part of the flow complex_snooze -> complex_snooze_is_in_condition->complex_snooze_finish.

    The purpose of the function is to see if the condition of the complex snooze is still met -
    that is, whether the sugar values are above 140 and below the target value defined when running the complex_snooze (the value from which we want to run the prediction algorithm again).
    """
    job = context.job
    high_val ,chat_id = job.data
    ...
    # If the sugar level is in the desired range (between 140 and the upper limit set in complex_snooze, continue in the loop - The complex snooze conditions still apply
    while curr_val<high_val and curr_val>140 :
        await asyncio.sleep(2*60+30)
        # print("waited 2.5 min")
        curr_val = predKalman.currnet_bg_val(collection_mongo,1)
    
    # If the threshold is crossed in either direction, continue in flow to end the complex snooze to the function complex_snooze_finish
    context.job_queue.run_once(complex_snooze_finish, 1, chat_id=chat_id, name=str(chat_id), data=[high_val,chat_id] )
...
/start_predict -> starts running the prediction algorithm.
/stop_predict -> stops the prediction algorithm.
/simple_snooze -> stops the algorithm for 30 minutes and then excutes it again.
/complex_snooze 'number' -> stops the notifications until reaching the 'number' or getting bellow 140.
...
async def start_pred_algorithm(update: Update, context = ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = "Start prediction algorithm"
    if job_removed:
...
async def terminate_pred_algorithm (update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Updating the user
    text = "prediction successfully cancelled!" if job_removed else "You have no active prediction algorithm running."
    await update.message.reply_text(text)


async def complex_snooze(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """"
    ...
    Function as part of the flow complex_snooze -> complex_snooze_is_in_condition->complex_snooze_finish.
    """
    global total_pred
    chat_id = update.message.chat_id
    ...
    try:high_border = int(context.args[0])
        ...
        await update.message.reply_text("didn't recive any values for the higher border, next time call /complex_snooze <number>")
        return    
    ...
    await update.message.reply_text(text)
    context.job_queue.run_once(complex_snooze_is_in_condition, 1, chat_id=chat_id, name=str(chat_id), data=[high_border,update.message.chat_id])
...
async def simple_snooze(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Returning an answer to the user and adding a task of starting the predictions algorithm in another 30 minutes
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("complex_snooze", complex_snooze))
